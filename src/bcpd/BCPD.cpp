...
namespace {
    ...
    void writePly(const std::vector<Eigen::Vector<FloatType, Dim>>& points, 
        ...
        file.add_properties_to_element("vertex", {"x", "y", "z"},
            tinyply::Type::FLOAT64, points.size(), 
    ...
    [[nodiscard]] FloatType digamma(FloatType x_in) noexcept {
        ...
        const double t = f * (-1.0/12.0 + f * (1.0/120.0 + f * 
            (-1.0/252.0 + f * (1.0/240.0 + f * (-1.0/132.0 + f * 
    ...
    template <typename FloatType>
    [[nodiscard]] FloatType calculateP(FloatType xyDist2, FloatType residual2, 
                                       FloatType alpha, FloatType omega) noexcept {
        constexpr FloatType TWO_PI = 2.0 * std::numbers::pi_v<FloatType>;
        const FloatType phi_mn = (1.0 / std::sqrt(TWO_PI * residual2)) * 
                                  std::exp(-xyDist2 / (2.0 * residual2));
...
void calculateNystromApprox(
    ...
    Eigen::JacobiSVD<MatrixType> svd(kernelMat, 
...
void BCPD<FloatType, Dim>::computeExpectationKdTree(uint32_t N, uint32_t M) {
    ...
    for (uint32_t m = 0u; m < M; ++m) {
        ...
        if (ret_matches.empty()) {
            ...
            xKdTree->query(y_hat[m].data(), NEAREST_NEIGHBORS_FALLBACK, 
...
void BCPD<FloatType, Dim>::updateResidual(uint32_t N, uint32_t M) {
    ...
    Eigen::JacobiSVD<MatrixType, Eigen::ComputeFullU | Eigen::ComputeFullV> 
        svd(Sxu, Eigen::ComputeFullU | Eigen::ComputeFullV);
