...
public:
    ...
    explicit BCPD(FloatType beta, FloatType lambda, FloatType omega, FloatType gamma) noexcept
        : m_beta(beta), m_lambda(lambda), m_omega(omega), m_gamma(gamma)
    {
    }
    ...
    void SetInput(const std::vector<VectorType>& x_in, const std::vector<VectorType>& y_in) noexcept
    {
        x = x_in;
        y = y_in;
    }
...
protected:
    ...
    void computeMaximizationNystrom(uint32_t M, FloatType cc, const std::vector<VectorType>& E);
    void computeMaximizationDirect(uint32_t M, FloatType cc, const std::vector<VectorType>& E);
